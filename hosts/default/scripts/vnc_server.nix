...
connect_avnc() {
    adb -s $serial_num shell input keyevent KEYCODE_WAKEUP
    sleep 1
    adb -s $serial_num shell input swipe 500 1500 500 500
    sleep 1
    adb -s $serial_num shell input text 1246
    sleep 1
    adb -s $serial_num shell input keyevent KEYCODE_ENTER
    sleep 1
    adb -s $serial_num shell monkey -p com.gaurav.avnc -c android.intent.category.LAUNCHER 1
    sleep 2
    adb -s $serial_num shell input tap 360 300
    sleep 1
    adb -s $serial_num shell input tap 360 1150
}

if [[ -n "$(pgrep -a wayvnc)" ]]; then 
    pkill wayvnc
    adb -s $serial_num shell input keyevent KEYCODE_SLEEP
...
if [[ "$1" == "two_monitors" ]]; then
    ...
    hyprctl output create headless phone_monitor_wifi
    adb -s $serial_num reverse tcp:5900 tcp:5900
