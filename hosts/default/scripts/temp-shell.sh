...
update_waybar() {
    ...
    taskbar_border_line="#taskbar { border-left: 5px double shade(@cursor, 0.7); }"
...
EOF
        ...
        cat > /home/$USER/.cache/wal/waybar_css_mutable.css <<EOF
            ...
            #workspaces { border-left: 5px double shade(@cursor, 0.7); border-right: 5px double shade(@cursor, 0.7); }
            #custom-timer { border-left: 5px double shade(@cursor, 0.7); }
            #mpris { border-left: 5px double shade(@cursor, 0.7); }
            $taskbar_border_line
            #tray { border-right: 5px double shade(@cursor, 0.7); }
            #language { border-right: 5px double shade(@cursor, 0.7); }
            #custom-weather { border-right: 5px double shade(@cursor, 0.7); }
